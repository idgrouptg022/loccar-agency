...
export default function RejectedInvoice(props) {
    ...
    return (
        <Fragment>
            ...
            <Grid container>
                <Grid xs={6} item>
                    <h2>Liste des devis rejetées</h2>
                </Grid>
            </Grid><br />
            {
                !props.userRights?.split(',').includes('15') && JSON.parse(localStorage.getItem('data')).id != 1 ?
                    ...
                    !isFetched ?
                        ...
                        <Fragment>
                            {
                                invoices.length === 0 ?
                                    <Typography textAlign={'center'} color={'GrayText'}><strong>Pas de nouveau devis rejeté</strong>.</Typography> :
